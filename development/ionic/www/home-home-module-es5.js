...
var HomePage = /** @class */ (function () {
    function HomePage(actionSheetController, http) {
    ...
    HomePage.prototype.ngOnInit = function () {
        var _this = this;
        var chartDataPromise = this.http.get('http://161.35.76.247:4001/demo').toPromise();
        chartDataPromise.then(function (resp) {
            console.log(resp);
            _this.avg_temp = resp;
        });
        var chartDataPromise2 = this.http.get('http://161.35.76.247:4001/demo2').toPromise();
        chartDataPromise2.then(function (resp) {
            console.log(resp);
            _this.run_pred.push(['Hour', 'Km']);
            for (var _i = 0, _a = resp; _i < _a.length; _i++) {
                var pair = _a[_i];
                _this.run_pred.push([pair[1], pair[0]]);
            }
            console.log(_this.run_pred);
            _this.chart.dataTable = _this.run_pred;
